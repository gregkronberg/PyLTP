...
class SlopeFuncs:
    ...
    def _remove_exp_fit(self, group_df, ):
        ...
        for i in group_df.index:
            ...
            if not param_range[0] < fit_coeff[1] < param_range[1]:
                group_df.at[i, 'exp_fit_remove'] = True
            elif exp_fit_error>error_threshold:
                group_df.at[i, 'exp_fit_remove'] = True
            else:
                group_df.at[i, 'exp_fit_remove'] = False
        print group_df.reset_index().set_index('field_polarity_0').loc['control'].remove
        return group_df
...
class Slopes:
    ...
    def _process(self, directory='Variables/', filename='slopes_df.pkl', pre_directory='Preprocessed Data/', **kwargs):
        ...
        # add experimenter column
        self.group_df['experimenter']='mahima'
        self.group_df['remove']=self.group_df['exp_fit_remove']
        self.group_df['baseline_percent']=40
